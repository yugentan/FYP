...
public class m5x24large implements INode {
    private Host host;
    private int id;
    //Characteristics of m5.x24large
    private static final String VM_TYPE = "m5.x24large";
    private static final int VM_MIPS = 1000; // (Millions Instruction per Second) Not disclosed, using default of 1000
    private static final int VM_RAM = 384000; // RAM: x24large instance have 1 GiB ram
    private static final int VM_BANDWIDTH = 100; // Bandwidth: x24large instance have low(100mb) - moderate(1000mb) bandwidth
    private static final int VM_PES = 96; //Number of Processing Elements (cores): x24large instance have 2 CPU

    private static final int VM_SIZE = 10000; //Storage in Mb: x24large instance does not come with storage, default of 10000Mb is used

    private static final double VM_COST = 4.61; //Cost: on demand cost for x24large instance
    public m5x24large(int id){
        this.id = id;
    }
