...
public class t2large implements INode {
    private Host host;
    private int id;
    //Characteristics of t2.large
    private static final String VM_TYPE = "t2.large";
    private static final int VM_MIPS = 1000; // (Millions Instruction per Second) Not disclosed, using default of 1000
    private static final int VM_RAM = 8000; // RAM: large instance have 1 GiB ram
    private static final int VM_BANDWIDTH = 100; // Bandwidth: large instance have low(100mb) - moderate(1000mb) bandwidth
    private static final int VM_PES = 2; //Number of Processing Elements (cores): large instance have 1vCPU

    private static final int VM_SIZE = 10000; //Storage in Mb: large instance does not come with storage, default of 10000Mb is used

    private static final double VM_COST = 0.09; //Cost: on demand cost for large instance
    public t2large(int id){
        this.id = id;
    }
